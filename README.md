...
|--------|-------------|---------|----------|
| [android-project-starter](plugins/android-project-starter/) | Wizard-driven Android project scaffolder. Multi-module MVI + Compose + Koin + Navigation 3, with build-logic convention plugins (incl. qa/prod product flavors), shake/broadcast dev-tools dialog, tests, CI, and project-local planner/implementer skills. | 0.4.0 | productivity |
| [android-project-aligner](plugins/android-project-aligner/) | Brownfield companion to android-project-starter. Audits an existing Android project against the starter's conventions, produces a phased migration plan, and applies it on a fresh git branch with build verification between phases. Also generates the project-local planner/implementer skills. | 0.2.0 | productivity |
| [code-review](plugins/code-review/) | Outside-in, risk-driven code review. Covers bug fixes, new features, add-ons, extensions, and refinements. Re-review aware, with a single summary comment per PR. | 1.4.0 | productivity |
| [dep-update-merge](plugins/dep-update-merge/) | Bundles dependency-update PRs/MRs into one verified change with changelog analysis and breaking change triage. | 1.2.0 | productivity |
| [feature-development](plugins/feature-development/) | End-to-end feature development: frame → explore → plan → implement (TDD) → verify → hand off → compound. Works with plan mode. | 0.5.0 | productivity |
